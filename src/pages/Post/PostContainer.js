    ...
    <div className='border rounded-xl p-6 sm:p-10 shadow-md space-y-4'>
      ...
      <div>
        <p className='text-xs font-light text-gray-400'>
          Posted by <Link className='text-blue-700 hover:text-blue-900' to={`/users/${post.postedBy._id}`}>{post.postedBy.profile.username}</Link> on {formatDate(post.datePosted)}
        </p>
        {post.dateEdited &&
